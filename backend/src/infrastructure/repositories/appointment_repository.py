from src.infrastructure.database.schemas import Appointment
...
class AppointmentRepository:
    ...
    async def get_available_appointments(self, professional_id: str, date: str):
        ...
        return []
        """get_one_stmt = select(Appointment).where(
            Appointment.start_time == start_time, Appointment.professional_id == professional_id)\
                .limit(1)
        result = (await self.session.execute(get_one_stmt)).fetchone()
        if result:
            return result[0].available
